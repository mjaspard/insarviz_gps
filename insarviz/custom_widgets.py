...
from PyQt5.QtWidgets import QCheckBox, QTreeWidget, QTreeWidgetItem
...
class AnimatedToggle(Toggle):
    ...
    def paintEvent(self, e: QPaintEvent):
        ...
        p.setPen(Qt.black)
        p.drawText(contRect, (Qt.AlignCenter | Qt.AlignBottom), 'bkgd')

        p.end()


class FileInfoWidget(QTreeWidget):
    def __init__(self):
        super().__init__()

    def update_fileinfo(self, info):
        """
        called when file is loaded, gets file info from metadata, shows them
        in the "Contents" treewidget

        Parameters
        ----------
        info : tuple
            (filename with path, profile) sent by loader, with profile a dict
            type containing file info
        ...
        """
        filepath, profile = info
        # split path/filename
        filename = filepath.split('/')[-1]
        path = filepath[:-len(filename)]
        # get info from profile from loader:
        vals = [filename]
        for attr in profile:
        ...
        self.setHeaderLabels(['filename']+list(profile.keys())+['path'])
